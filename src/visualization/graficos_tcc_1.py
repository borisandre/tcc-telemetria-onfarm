...
def plot_radar(decision: pd.DataFrame, ahp: pd.DataFrame) -> None:
    criteria = [c for c in decision.columns if c != "hardware"]
    df = decision.copy()

    benefit_criteria = {
        "Conectividade": True,
        "Qualidade_Posicionamento": True,
        "Latencia": False,  # custo: menor = melhor
        "Qualidade_Dados": True,
        "Aderencia_Operacional": True,
        "Complexidade_Integracao": False,  # custo: menor = melhor
    }

    pesos_map = dict(zip(ahp["criterio"], ahp["peso"]))

    # Normalização 0-1
    norm_df = df.copy()
    for col in criteria:
        s = pd.to_numeric(norm_df[col], errors="coerce").astype(float)
        min_v = s.min()
        max_v = s.max()
        ...
        if benefit_criteria.get(col, True):
            norm_df[col] = (s - min_v) / (max_v - min_v)
        else:
            norm_df[col] = (max_v - s) / (max_v - min_v)

    # Aplicação dos pesos AHP
    weighted_df = norm_df.copy()
    for col in criteria:
        weighted_df[col] = weighted_df[col] * pesos_map[col]
    ...
    ax.set_title("Perfil comparativo ponderado das alternativas por critério", y=1.10)
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(labels)

    max_peso = max(pesos_map.values())
    ax.set_ylim(0, max_peso)
    ticks = np.linspace(max_peso / 4, max_peso, 4)
    ax.set_yticks(ticks)
    ax.set_yticklabels([f"{v:.2f}".replace(".", ",") for v in ticks])

    ax.legend(loc="upper right", bbox_to_anchor=(1.28, 1.10), frameon=False)

    save_fig(fig, "fig_08_radar_criterios_ponderado.png")


def plot_latency_boxplot(metrics_session: pd.DataFrame) -> None:
    plot_df = metrics_session.copy()

    # Descobre automaticamente a melhor coluna de latência disponível
    latency_candidates = ["avg_delay_ms", "delay_avg_ms", "latency_ms"]
    latency_col = next((c for c in latency_candidates if c in plot_df.columns), None)

    if latency_col is None:
        raise KeyError(
            f"Nenhuma coluna de latência encontrada. Esperado uma entre: {latency_candidates}"
        )

    plot_df[latency_col] = pd.to_numeric(plot_df[latency_col], errors="coerce")
    plot_df = plot_df.dropna(subset=[latency_col, "hardware"])

    order = ["FMC920", "ZEFIRO", "ABS"]
    data = [plot_df.loc[plot_df["hardware"] == hw, latency_col].values for hw in order]
...
def plot_error_bars(metrics_session: pd.DataFrame) -> None:
    plot_df = metrics_session.copy()
    plot_df["avg_delay_ms"] = pd.to_numeric(plot_df.get("avg_delay_ms"), errors="coerce")
    plot_df["gps_valid_rate_pct"] = pd.to_numeric(plot_df.get("gps_valid_rate_pct"), errors="coerce")
