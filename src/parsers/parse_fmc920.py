...
RAW_ROOT = Path("data/raw_samples/raw/fmc920")
OUTPUT_DIR = Path("data/parsed")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

OUTPUT_PARQUET = OUTPUT_DIR / "fmc920_parsed.parquet"
OUTPUT_CSV = OUTPUT_DIR / "fmc920_parsed.csv"
...
def to_float(value: Any) -> Optional[float]:
    ...
    # suporte leve para decimal com vírgula
...
def parse_fmc920_record(envelope: Dict[str, Any]) -> Dict[str, Any]:
    normalized = {
    ...
    # caso a própria linha JSONL esteja inválida
    ...
    # raw_payload pode vir como dict ou como string JSON
    raw_payload = envelope.get("raw_payload")
    normalized["raw_payload"] = raw_payload if isinstance(raw_payload, str) else json.dumps(raw_payload, ensure_ascii=False)
    ...
    if isinstance(raw_payload, dict):
        ...
        try:
            ...
            normalized["parse_error"] = "invalid_raw_payload_json"
            flags = ["invalid_raw_payload_json"]
            normalized["data_quality_flags"] = ";".join(flags)
            return normalized
    else:
    ...
    # extração do payload nativo FMC920
    device_id = payload_obj.get("deviceId")
    event_time = parse_datetime(payload_obj.get("ts"))

    latitude = to_float(safe_get(payload_obj, "gps", "lat"))
    longitude = to_float(safe_get(payload_obj, "gps", "lon"))
    gps_fix_raw = safe_get(payload_obj, "gps", "fix")
    gps_valid = to_int01(gps_fix_raw)
    speed_kmh = to_float(safe_get(payload_obj, "gps", "speed_kmh"))
    rssi_dbm = to_float(safe_get(payload_obj, "gsm", "rssi_dbm"))

    machine_on = to_int01(safe_get(payload_obj, "io", "din1_ignition"))
    machine_operating = to_int01(safe_get(payload_obj, "io", "din_operating"))

    buffered = to_int01(payload_obj.get("buffered"))
    ...
    normalized["is_buffered"] = buffered

    # session_id
    # por enquanto usamos hardware_session_id se existir
    # se não existir, cai para device_id + data
    ...
    # gps status
    if gps_valid == 1:
        normalized["gps_status"] = "valid"
    elif gps_valid == 0:
        normalized["gps_status"] = "invalid"
    else:
        normalized["gps_status"] = "unknown"
...
def main() -> None:
    ...
    for file in files:
        ...
        for row in rows:
            parsed_rows.append(parse_fmc920_record(row))
    ...
    if "data_quality_flags" in df.columns:
        print(df["data_quality_flags"].fillna("SEM_FLAG").value_counts().head(10).to_string())
